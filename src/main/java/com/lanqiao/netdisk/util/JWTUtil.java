...
import com.lanqiao.netdisk.config.jwt.JwtProperties;
...
public class JWTUtil {
    ...
    private SecretKey generalKey() {
        System.out.println("当前JWT密钥"+jwtProperties.getSecret());
        // 本地的密码解码
    ...
    public String createJWT(String subject) throws Exception {
        ...
        // 生成签名的时候使用的秘钥secret，切记这个秘钥不能外露，是你服务端的私钥，
        // 在任何场景都不应该流露出去，一旦客户端得知这个secret，那就意味着客户端是可以自我签发jwt的
        ...
        JwtBuilder builder = Jwts.builder()
                .setClaims(defaultClaims)//声明
                .setIssuedAt(nowDate)//token的生成时间
                .signWith(SignatureAlgorithm.forName(jwtProperties.getHeader().getAlg()),key);//选择加密方式和key
