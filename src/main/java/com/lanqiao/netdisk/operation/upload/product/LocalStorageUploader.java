package com.lanqiao.netdisk.operation.upload.product;

import com.lanqiao.netdisk.exception.NotSameFileExpection;
import com.lanqiao.netdisk.exception.UploadException;
...
public class LocalStorageUploader extends Uploader {
    ...
    public List<UploadFile> upload(HttpServletRequest httpServletRequest, UploadFile uploadFile) {
        ...
        //StandardMultipartHttpServletRequest 标准多部件Servlet请求
        ...
        if (!isMultipart) {
            throw new UploadException("未包含文件上传域");  //自定义文件上传异常
        }
    ...
    private List<UploadFile> doUpload(StandardMultipartHttpServletRequest standardMultipartHttpServletRequest,
                                      ...
                                      UploadFile uploadFile) throws IOException, NotSameFileExpection {
        List<UploadFile> saveUploadFileList = new ArrayList<UploadFile>();
        ...
        String fileType = FileUtil.getFileExtendName(originalName); //使用自写文件工具类获取文件扩展名
